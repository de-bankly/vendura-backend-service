  ...
  public static SupplierOrderDTO toDTO(SupplierOrder supplierOrder) {
    return new SupplierOrderDTO(
        ...
        supplierOrder.getPositions().stream()
            .map(
                position ->
                    new SupplierOrderDTO.Position(
                        ProductFactory.toDTO(position.getProduct()), position.getAmount()))
            ...
            .map(
                position ->
                    new SupplierOrder.Position(
                        ProductFactory.toEntity(position.getProduct()), position.getAmount()))
            .collect(Collectors.toSet()),
